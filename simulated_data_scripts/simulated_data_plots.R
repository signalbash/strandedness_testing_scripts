...
write.table(strand_results[,c(18, 5,6,7,4,8:15, 17)], file="hawsh_strandedness_results.txt", sep="\t", row.names = F)
...
for(s in seq_along(species)){
  ...
  p3=
    ggplot(strand_results_sim_species_n[strand_results_sim_species_n$percent_stranded==0,], 
         aes(x=factor(n_reads), y=max_strand_no_fail,fill=factor(percent_stranded))) + 
    geom_boxplot() + facet_wrap(~species_sci) + theme_figure + scale_x_discrete("Number of reads") + 
    scale_y_continuous("how_are_we_stranded_here\nstranded proportion") +
    theme(axis.text.x = element_text(angle = 90, hjust = 1), legend.position = "none") 
  
  ggplot(strand_results_sim_species_n[strand_results_sim_species_n$percent_stranded==1,], 
         aes(x=factor(n_reads), y=max_strand_no_fail,fill=factor(percent_stranded))) + 
    geom_boxplot() + facet_wrap(~species_sci) + theme_figure + scale_x_discrete("Number of reads") + 
    scale_y_continuous("how_are_we_stranded_here\nstranded proportion") +
    theme(axis.text.x = element_text(angle = 90, hjust = 1), legend.position = "none") 
  
  aggregate(max_strand_no_fail ~ n_reads+percent_stranded+species, 
            strand_results_sim_species_n, function(x) sd(x)*100*3)
  ...
  pdf("suppl_fig_simulated.pdf", height=6,width=6.5)  
ggdraw() +
  draw_plot(p2 + theme(legend.position = c(0.8, 0.6)), 0,0.66,1,0.33)+
  draw_plot(p3, 0,0.33,1,0.33) + 
  draw_plot(p1, 0,0.00,1,0.33)+
  draw_plot_label(c("A","B","C"), x=c(0,0,0), y=c(1,0.66,0.33), size=12)
dev.off()
